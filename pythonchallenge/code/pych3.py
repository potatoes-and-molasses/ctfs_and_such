...
import urllib
a=a.replace('\n','')
iscool = lambda x: 1 if all([i in string.ascii_uppercase for i in x[1:4]]+[i in string.ascii_uppercase for i in x[5:8]]+[x[i] in string.ascii_lowercase for i in [0,4,8]]) else 0
for i in range(len(a)-9):
    stuff=a[i:i+9]
    
    if iscool(stuff):
        stuff=stuff[4:-4]
        print 'http://www.pythonchallenge.com/pc/def/{}.html'.format(stuff)
        page=urllib.urlopen('http://www.pythonchallenge.com/pc/def/{}.html'.format(stuff)).read()
        if '<title>404 Not Found</title>' not in page:
            print 'good'
        else:
            print '404'
